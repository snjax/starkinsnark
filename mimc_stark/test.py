...
def test_stark():
    INPUT = 3
    import sys
    LOGSTEPS = int(sys.argv[1]) if len(sys.argv) > 1 else 4
    # Full STARK test
    import random
    ...
    print(list(map(len, fri_proof)))
    print(fri_proof[1])
    print(list(map(len, fri_proof[0])))
    print(fri_proof[0][0])
    print(list(map(len, fri_proof[0][1])))
    print(list(map(len, fri_proof[0][2])))
    print(fri_proof[0][1][0])
    print(fri_proof[0][2][0])
